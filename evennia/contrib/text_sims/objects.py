...
class CmdRead(Command):
    ...
    def func(self):
        ...
        reward = self.obj.at_read()
        self.caller.msg("You read %s.\n[REWARD_read : %d]" % (obj.name, reward))
        # set a tag on the caller to remember that we read.
        self.caller.tags.add("tutorial_read", category="text_sims")
...
class Readable(TutorialObject):
    """
    A readable object. All that is special about it is that it has
    the "read" command available on it.
    """
    ...
    def at_object_creation(self):
        ...
        self.read_reward = 1 #default value

    def at_read(self):
        return self.read_reward
